import { pgTable, varchar, text, integer, boolean, serial, uuid, date, timestamp } from "drizzle-orm/pg-core";
...
export const scoutInvestorsRelations = relations(scoutApproved, ({ one }) => ({
    scout: one(scouts, { fields: [scoutApproved.scoutId], references: [scouts.scoutId] }),
    investor: one(users, { fields: [scoutApproved.investorId], references: [users.id] }),
}));
