...
export const daftarInvestors = pgTable("daftar_investors", {
    ...
    daftarId: varchar("daftar_id", { length: 255 }).references(() => daftar.id, { onDelete: "cascade" }),
    investorId: varchar("investor_id", { length: 255 }).references(() => users.id, { onDelete: "cascade" }),
...
export const daftarInvestorsRelations = relations(daftarInvestors, ({ one }) => ({
    daftar: one(daftar, { fields: [daftarInvestors.daftarId], references: [daftar.id] }),
    investor: one(users, { fields: [daftarInvestors.investorId], references: [users.id] }),
}));
