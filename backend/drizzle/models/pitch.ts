import {
    pgTable, varchar, text, integer, boolean, serial, uuid, date, timestamp
...
export const pitch = pgTable("pitch", {
    ...
    scoutId: varchar("scout_id", { length: 255 }).references(() => scouts.scoutId),
...
export const offerActions = pgTable("offer_actions", {
    ...
    actionTakenBy: varchar("action_taken_by", { length: 255 }).references(() => users.id),
    actionTakenAt: timestamp("action_taken_at"),
});

// 🎤 Investor Answers
export const investorAnswers = pgTable("investor_answers", {
    id: serial("id").primaryKey(),
    pitchId: varchar("pitch_id", { length: 255 }).references(() => pitch.id),
    pitchAnswerUrl: text("pitch_answer_url").notNull(),
    questionId: varchar("question_id", { length: 255 }).references(() => scouts.scoutId),
...
export const investorPitch = pgTable("investor_pitch", {
    ...
    pitchId: varchar("pitch_id", { length: 255 }).references(() => pitch.id),
    scoutId: varchar("scout_id", { length: 255 }).references(() => scouts.scoutId),
    investorId: varchar("investor_id", { length: 255 }).references(() => users.id),
...
export const pitchRelations = relations(pitch, ({ many, one }) => ({
    ...
    offers: many(offers),
    investorAnswers: many(investorAnswers),
...
export const investorAnswerRelations = relations(investorAnswers, ({ one }) => ({
    pitch: one(pitch, { fields: [investorAnswers.pitchId], references: [pitch.id] }),
    question: one(scouts, { fields: [investorAnswers.questionId], references: [scouts.scoutId] }),
}));
...
export const pitchDeleteRelations = relations(pitchDelete, ({ one }) => ({
    ...
    founder: one(users, { fields: [pitchDelete.founderId], references: [users.id] }),
}));

export const investorPitchRelations = relations(investorPitch, ({ one }) => ({
    pitch: one(pitch, { fields: [investorPitch.pitchId], references: [pitch.id] }),
    scout: one(scouts, { fields: [investorPitch.scoutId], references: [scouts.scoutId] }),
    investor: one(users, { fields: [investorPitch.investorId], references: [users.id] }),
}));