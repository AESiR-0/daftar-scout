...
export default function PitchBoardPage() {
  ...
  const [groupedPitches, setGroupedPitches] = useState<Record<string, Pitch[]>>(
    () =>
      daftarsData
        .flatMap((daftar) => daftar.pitches) // Extract pitches from all daftars
        .reduce((acc, pitch) => {
          if (!acc[pitch.status]) {
            acc[pitch.status] = [];
          }
  ...
  const hasPitches = Object.values(groupedPitches).some(
                  ...
                  <ScrollArea className="h-[calc(100vh-16rem)]">
                    <div className="space-y-3">
                      {statusPitches.map((pitch) => (
                        <Link key={pitch.id} href={`/founder/studio`}>
                          <div
                            className="p-4 rounded-[0.35rem] mb-2 bg-background border 
                              ...
                              <div>
                                ...
                                <div className="flex flex-col gap-1 mt-1">
                                  <p className="text-xs text-muted-foreground mb-2">
                                    Collaboration: Nithin Kamath, Piyush Bansal,
                                    Aman Gupta
                                  </p>
