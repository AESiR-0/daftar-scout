...
export async function POST(req: NextRequest) {
  try {
    ...
    const updatedScout = await db
      .update(scouts)
      .set({
        targetAudLocation: targetAudLocation || null,
        targetAudAgeStart: targetAudAgeStart || null,
        targetAudAgeEnd: targetAudAgeEnd || null,
        scoutCommunity: scoutCommunity || null,
        scoutStage: scoutStage || null,
        scoutSector: scoutSector || null,
      })
      .where(eq(scouts.scoutId, scoutId))
      .returning();
