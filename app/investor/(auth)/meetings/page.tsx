...
export default function MeetingsPage() {
  ...
  // Simulate fetching meetings (replace with actual API call)
  useEffect(() => {
    const fetchMeetings = async () => {
      try {
        ...
        // const res = await fetch("/api/endpoints/meetings", {
        //   method: "GET",
        //   headers: {
        ...
        // if (res.status !== 200) throw new Error("Failed to fetch meetings");
        // setMeetings(data); // If using dynamic state
  ...
  if (loading) {
    return (
      <div className="space-y-6 container mx-auto px-10 bg-[#0e0e0e]">
        {/* Skeleton for Header */}
        <div className="flex items-center justify-between">
          <Skeleton className="h-7 w-32 bg-[#2a2a2a] rounded-[0.35rem]" />
          <Skeleton className="h-9 w-36 bg-[#2a2a2a] rounded-[0.35rem]" />
        </div>
        {/* Skeleton for Three Column Layout */}
        <div className="flex gap-6">
          {/* Calendar and Pending Column */}
          <div className="space-y-6 flex-1">
            <Skeleton className="h-[300px] w-full bg-[#2a2a2a] rounded-[0.35rem]" />
          ...
          <div className="w-1/3 border-l px-6 space-y-4">
            <div className="flex items-center justify-between">
              <Skeleton className="h-6 w-28 bg-[#2a2a2a]" />
            </div>
            <Skeleton className="h-16 w-full bg-[#2a2a2a] rounded-[0.35rem]" />
            <Skeleton className="h-16 w-full bg-[#2a2a2a] rounded-[0.35rem]" />
          </div>
          {/* Meeting Details Column */}
          <div className="border-l px-6 flex-1">
            <Skeleton className="h-8 w-48 bg-[#2a2a2a] rounded-[0.35rem]" />
            <div className="space-y-4 mt-4">
              <Skeleton className="h-5 w-32 bg-[#2a2a2a]" />
              <Skeleton className="h-5 w-64 bg-[#2a2a2a]" />
              <Skeleton className="h-5 w-48 bg-[#2a2a2a]" />
              <Skeleton className="h-5 w-56 bg-[#2a2a2a]" />
              <Skeleton className="h-20 w-full bg-[#2a2a2a] rounded-[0.35rem]" />
              <div className="flex gap-2">
                <Skeleton className="h-8 w-20 bg-[#2a2a2a] rounded-[0.35rem]" />
                <Skeleton className="h-8 w-20 bg-[#2a2a2a] rounded-[0.35rem]" />
                <Skeleton className="h-8 w-20 bg-[#2a2a2a] rounded-[0.35rem]" />
              </div>
            </div>
            ...
            {selectedDateMeetings.length === 0 ? (
              ...
              selectedDateMeetings.map((meeting) => (
                <div
                  ...
                  onClick={() => setSelectedMeeting(meeting)}
                >
                  <div className="w-20 text-sm font-medium">{meeting.time}</div>
              ...
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Badge variant="secondary">{selectedMeeting.status}</Badge>
                ...
                <div className="flex items-center gap-2">
                  ...
                  {selectedMeeting.createdBy === currentUser && (
                    <Button
                      size="sm"
                      variant="outline"
                      className="text-red-600 hover:text-red-700 rounded-[0.35rem]"
